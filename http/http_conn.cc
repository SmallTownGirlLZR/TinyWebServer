...
bool http_conn::add_response(const char *format, ...) {
  ...
  int n = vsnprintf(m_write_buf, WRITE_BUFFER_SIZE - m_write_idx - 1, format,
                    arg_list);
...
http_conn::HTTP_CODE http_conn::parse_headers(char *text) {
  if (text[0] ==
      '\0') { /* 不意味 请求头没有任何内容，而是解析到了请求头的末尾（空行）*/
...
http_conn::HTTP_CODE http_conn::process_read() {
  ...
  while ((m_check_state == CHECK_STATE_CONTENT && line_status == LINE_OK) ||
    ...
    case CHECK_STATE_CONTENT: {
      ...
      line_status =
          LINE_OPEN; /* parse_content没有返回GET_REQUEST，则说明还有数据要继续读取*/
      break;
    }
    }
  }
    ...
    if (temp < 0) {
      if (errno == EAGAIN) { /* 缓冲区被写满，不再允许写入 */
        modfd(m_epollfd, m_sockfd, EPOLLOUT,
              m_TRIGMode); /* EPOLLOUT 下次缓冲区有剩余空间时，epoll通知 */
        return true;
      }
      unmap();
      return false;
