...
bool http_conn::add_response(const char* format, ...){
    ...
    int n = vsnprintf(m_write_buf,WRITE_BUFFER_SIZE - m_write_idx - 1,format,arg_list);
...
http_conn::LINE_STATUS http_conn::parse_line(){
    ...
    for(;m_checked_idx < m_read_idx;m_checked_idx++){
        ...
        if(temp == '\r'){
            if(m_checked_idx + 1 == m_read_idx){return LINE_OPEN;}      /* \r 是最后一个字符，需要继续读取 */
...
bool http_conn::read_once(){
    ...
    if(m_TRIGMode == 0){    /* LT 模式*/
        ...
        if(bytes_have_send <= 0)    return false;
...
http_conn::HTTP_CODE http_conn::parse_request_line(char* text){
    ...
    *m_url++ = '\0';    /* 空格转化为 \0 以获取method */
    ...
    if(strcasecmp(method,"GET") == 0){      /*  The  strcasecmp()  function performs a byte-by-byte comparison of the strings s1 and s2, ignoring the case of the characters.*/
        m_method = GET;
...
http_conn::HTTP_CODE http_conn::parse_content(char* text){
    ...
    */
    if(m_read_idx >= m_checked_idx + m_content_length){     /*m_checked_idx + m_content_length 即请求体的最后位置*/
        ...
        */
        ...
        if(*(p + 1) == 3){
            ...
            if(users.find(name) == users.end()){
                ...
                if(!res){
                    strcpy(m_url,"./log.html"); /* 此前的m_url已经拷贝至m_url_real 并连接到 m_real_file 中*/
                }else{
    ...
    while(true){
        ...
        */
        if(bytes_have_send >= m_iv[0].iov_len){
            ...
            /* 重置body 偏移量 
                bytes_have_send - m_write_idx 已发送数据 - header长度(header储存在m_write_buf)中
                bytes_have_send - m_write_idx 即为已经发送的body长度
            */
